...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.update_all_icon_images()
        self.stdout.write("Done!")

    def _load_card_expansions(self, card_id, raw_expansions_field):
    ...
    def load_library(self):
        with open_text(PACKAGE, 'vteslib.csv', encoding='utf8') as csv_library:
            ...
            for row in reader:
                ...
                lc.save()
                self._load_card_expansions(row[0], row[12])
    ...
    def load_crypt(self):
        with open_text(PACKAGE, 'vtescrypt.csv', encoding='utf8') as csv_crypt:
            ...
            for cc in crypt_cards:
                ...
                cc.save()
                self._load_card_expansions(cc.id, cc.publish_set)  # id - publish_set
    ...
    def update_all_icon_images(self):
        sets = list(Set.objects.all())
        parsed_sets = [zet.id for zet in sets]

        pool = ThreadPool(processes=4)
        results = pool.map(self.check_icon_image, parsed_sets)
        pool.close()
        pool.join()
